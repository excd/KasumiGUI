...
namespace KasumiGUI.Utility
{
    internal static class Logger
    {
        ...
        private static string FormatMessage(LogMessage message) => string.Format("[{0}][{1,8}][{2,8}] {3}", DateTime.Now, message.Source[..Math.Min(message.Source.Length, 8)], message.Severity, message.Message);
    }
}