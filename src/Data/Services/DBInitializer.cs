    ...
    internal class DBInitializer(DBContext botResponseContext)
    {
        ...
        private static async Task InitializeResourceAsync<T>(DbSet<T> dbSet, string fileName) where T : BaseResponse, new()
        {
            if (!await dbSet.AnyAsync())
            {
                ...
                {
                    await Logger.LogAsync(new LogMessage(LogSeverity.Warning, "DBInitializer", $"Resource file not found: {resourcePath}"));
                    return;
                }
                try
                {
                    ...
                    await Logger.LogAsync(new LogMessage(LogSeverity.Info, "DBInitializer", $"Initialized database resource from {fileName}"));
                }
                catch (Exception ex)
                {
                    await Logger.LogAsync(new LogMessage(LogSeverity.Error, "DBInitializer", $"Failed to initialize database: {ex.Message}", ex));
                }
            }
            else
            {
                await Logger.LogAsync(new LogMessage(LogSeverity.Verbose, "DBInitializer", $"Database resource already initialized, skipping {fileName}"));
            }
        }
